...
export default class StarReview extends Component {
  ...
  displayMode() {
    ...
    const spacingStyle = {
      paddingLeft: this.props.spacing / 2,
      paddingRight: this.props.spacing / 2
    };
    const stars = [];
    for (let i = 1; i < this.props.count + 1; i++) {
      if (i == Math.floor(this.displayValue) + 1) {
        ...
        const emptyStarComponent = this.isReactElement(this.props.emptyStar) ? (
          ...
          <View key={i} style={spacingStyle}>
            <Image style={starStyle} source={this.props.emptyStar} alt="Empty Star" />
          </View>
        );
        ...
        const starComponent = this.isReactElement(this.props.fullStar) ? (
          ...
          <View key={i} style={spacingStyle}>
            <Image style={starStyle} source={this.props.fullStar} alt="Filled Star" />
          </View>
        );
  ...
  createHalfStarMember(index, star, halfStar) {
    ...
    starComponent = this.isReactElement(starComponent) ? (
      ...
      <ImageBackground
        style={{ width: this.props.starSize, height: this.props.starSize }}
        source={star}
      >
        <Image
          style={{ width: this.props.starSize, height: this.props.starSize }}
...
StarReview.defaultProps = {
  fullStar: <Icon as={FontAwesome} name={'star'} color="warning.400" size={`${spacing[30]}px`} />,
  halfStar: (
    <Icon as={FontAwesome} name={'star-half-full'} color="warning.400" size={`${spacing[30]}px`} />
  ),
  emptyStar: <Icon as={FontAwesome} name={'star-o'} color="gray.200" size={`${spacing[30]}px`} />,
  disabled: false,
  display: null,
  count: 5,
  default: 0,
