...
const { createRunOncePlugin, AndroidConfig, withAndroidStyles } = require('@expo/config-plugins');
...
const ACCENT_COLOR = '#8331FF';

/** @type {import('@expo/config-plugins').ConfigPlugin} */
const withTheme = config =>
  withAndroidStyles(config, config => {
    const dialogDateParent = {
      name: DIALOG_PICKER_THEME_NAME,
      parent: 'Theme.AppCompat.Light.Dialog'
    };
    const dialogTimeParent = {
      name: TIME_PICKER_THEME_NAME,
      parent: 'Theme.AppCompat.Light.Dialog'
    };
...
module.exports = createRunOncePlugin(withTheme, 'withAndroidDatePickerColorConfig');
